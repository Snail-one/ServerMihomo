...
import (
	...
	"snailproxy/internal/ui/subscriptionmenu"
)
...
func selectSubscriptionTarget(actionName string, subscriptions []mihomo.Subscription) (int, bool, error) {
	...
	index, err := subscriptionmenu.SelectSubscription(subscriptionLabels(subscriptions))
	if err != nil {
		return -1, false, err
	}
	if index < 0 {
...
func downloadNewSubscription(ctx context.Context, store mihomo.Store, subscriptions []mihomo.Subscription) error {
	subscriptionURL, err := subscriptionmenu.PromptSubscriptionURL()
	if err != nil {
		return err
	}
	...
	name, err := subscriptionmenu.PromptSubscriptionName(mihomo.DefaultSubscriptionName(subscriptionURL, fileName))
	if err != nil {
		return err
	}
...
func updateExistingSubscription(ctx context.Context, store mihomo.Store, subscriptions []mihomo.Subscription, index int) error {
	...
	name, err := subscriptionmenu.PromptSubscriptionName(subscription.Name)
	if err != nil {
		return err
	}
...
func modifyExistingSubscription(ctx context.Context, store mihomo.Store, subscriptions []mihomo.Subscription, index int) error {
	...
	subscription := subscriptions[index]
	subscriptionURL, err := subscriptionmenu.PromptSubscriptionURLDefault(subscription.URL)
	if err != nil {
		return err
	}
	...
	name, err := subscriptionmenu.PromptSubscriptionName(defaultName)
	if err != nil {
		return err
	}
...
func deleteExistingSubscription(store mihomo.Store, subscriptions []mihomo.Subscription, index int) error {
	...
	confirmed, err := subscriptionmenu.ConfirmDeleteSubscription(label)
	if err != nil {
		return err
	}
	if !confirmed {
